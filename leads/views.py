...
from .models import Lead, Address, Category
...
class LeadListView(LoginRequiredMixin, generic.ListView):
    ...
    def get_queryset(self):
        ...
        if user.is_organisor:
            ...
            queryset = Lead.objects.all()
            address = Address.objects.filter(user = queryset.user)
...
def lead_detail(request, pk):
    ...
    address = Address.objects.get(username = lead.email)
    context = {
        "lead": lead,
        "address": address,
    }
...
# class LeadAddressView(OrganisorAndLoginRequiredMixin, generic.CreateView):
#     template_name = "leads/lead_create.html"
...
#         lead.save()
#         messages.success(self.request, "You have successfully created a lead")
#         return super(LeadAddressView, self).form_valid(form)
...
def lead_create(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid() and address.is_valid() :
            ...
            user_obj = User()
            address_obj = Address()
            address_obj.username = form.cleaned_data.get('email')
            ...
            user_obj.username = form.cleaned_data.get('email')
            user_obj.save()
            form.save()
            address_obj.save()
...
def lead_update(request, pk):
    ...
    if request.method == "POST":
        ...
        address = AddressModelForm(request.POST,)
        if form.is_valid() and address.is_valid():
            address_obj = Address.objects.get(username = lead.email)
            address_obj.username = form.cleaned_data.get('email')
            form.save()
            address_obj.save()
...
def lead_delete(request, pk):
    lead = Lead.objects.get(id=pk)
    address = Address.objects.filter(username = lead.email)
    user = User.objects.filter(username = lead.email)
    user.delete()
    address.delete()
    lead.delete()
    return redirect("/leads")
